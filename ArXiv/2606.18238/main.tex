...
The proof now follows a dimension-count strategy. \Cref{lem:graded-ring-lift}
gives a general lifting result for graded hypersurface rings.
\Cref{lem:X10-hypersurface-parameters} identifies hypersurface isomorphism
classes with \(G\)-orbits, while
\cref{lem:X10-hypersurface-stabilizers} identifies their stabilizers with
their automorphism groups.
\Cref{lem:X10-configuration-parameters} proves that the stabilizer in \(H\)
of a general ordered configuration is trivial, while
\cref{lem:X10-finite-presentations} proves the required finiteness of blow-up
presentations modulo \(H\) and of the relevant automorphism groups. Finally,
\cref{prop:X10-blowup-dominance} constructs \(F:G\times W_0\to U\) and
shows that every nonempty fiber has dimension \(8\). Hence
\[
\dim \overline{F(G\times W_0)}
=11+16-8
=19
=\dim U,
\]
...
\(Q\), which is irreducible by the discussion above. Consider the open
locus where all eight points lie in the chart \(u\ne0\)
and at least five have distinct images in \(\Pp^1_{[u:v]}\); these five
points will also provide the \(5\times5\) minor for the five residual
unknowns below. An
automorphism fixing the ordered tuple induces an element of
\(\operatorname{PGL}_2\) fixing at least five distinct points, and hence
the identity, since a nontrivial M\"obius transformation has at most two
fixed points. Thus \(A=\lambda I\) for some \(\lambda\in\C^\times\). The
weighted scalar represented by \(\lambda\) acts as
\((u,v,z)\mapsto(\lambda u,\lambda v,\lambda^3z)\); composing with its
inverse gives a representative with \(A=I\). After renaming the coefficient
of \(z\) and the binary cubic, the residual action in the affine coordinates
\(t=v/u\) and \(\zeta=z/u^3\) is still
\((t,\zeta)\mapsto(t,a\zeta+q(t))\), where \(\deg q\leq3\). Write
\(q(t)=c_0+c_1t+c_2t^2+c_3t^3\). The condition that
\((t_j,\zeta_j)\) be fixed is
\[
(a-1)\zeta_j+c_0+c_1t_j+c_2t_j^2+c_3t_j^3=0,
\qquad 1\leq j\leq 8.
\]
These equations form a homogeneous linear system in
\((a-1,c_0,c_1,c_2,c_3)\) with coefficient matrix
\[
M(P)=
\begin{pmatrix}
...
\end{pmatrix}.
\]
The rank-five condition is open, since its complement is the common zero
locus of the \(5\times5\) minors. To show that this open locus is nonempty,
choose five distinct values \(t_j\) and set \(\zeta_j=t_j^4\). The
corresponding minor is, up to sign, the Vandermonde determinant
\[
\det(t_j^{\ell-1})_{1\leq j,\ell\leq5}
=\prod_{1\leq i<j\leq5}(t_j-t_i)\ne0.
\]

The chart, distinctness, and rank-five conditions therefore define a
nonempty open subset \(\mathcal C^\circ\subset\mathcal C\). Since
\(W\subset\mathcal C\) is also nonempty open and \(\mathcal C\) is
irreducible, \(W\cap\mathcal C^\circ\) is nonempty open. For every tuple in
this intersection, the fixing equations give \(a=1\) and \(q=0\).
Thus the normalized representative is the identity, and the stabilizer in
\(H\) is trivial.
\end{proof}
...
quotients are line bundles. Choose local frames and lift them to local
sections \(x,y,z,w\) of degrees \(1,2,3,5\), respectively.
The chosen generators induce a degree-\(10\) evaluation map
\[
...
\(10\). After a further shrinking, the constant-rank property makes this
kernel a line bundle. Trivializing it locally gives a relation
\(f_P(x,y,z,w)=0\) whose coefficients vary regularly with \(P\).
Changing the trivialization multiplies all coefficients by the same
nowhere-vanishing scalar, so the projective class \([f_P]\) is independent
of this choice. Hence these coefficients define a morphism
\[
