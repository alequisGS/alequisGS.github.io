...
and reproduction instructions accompany this draft but are not yet deposited
in a permanent public repository.  The package contains
the primary and independent plateau certificates, the direct AP checker, the
two staircase-criterion programs, and their complete logs.  The directory
\compdir{} contains compilation
and reproduction commands in \path{README_COMPUTATIONS.md}; the generated
summary and checksums are \path{CERTIFICATE.txt} and \path{SHA256SUMS}.
These files will be placed in a permanent repository
before public circulation.

% TODO BEFORE SUBMISSION:
% Upload the computational source, logs, and reproduction instructions to a
% public GitHub repository; create an archival Zenodo release and replace the
% preceding sentence by the permanent repository URL and DOI.  The deposit
% should archive the complete computational_reproducibility_files directory.
